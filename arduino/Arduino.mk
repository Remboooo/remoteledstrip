...
ifeq ($(words $(call rwildcard, src/, *.pde) $(call rwildcard, src/, *.ino)), 0)
$(error No pde or ino sketch)
endif

ifneq ($(words $(call rwildcard, src/, *.pde) $(call rwildcard, src/, *.ino)), 1)
...
# Local sources
#
LOCAL_C_SRCS    += $(call rwildcard, src/, *.c)
LOCAL_C_SRCS    := $(sort $(LOCAL_C_SRCS))
LOCAL_CPP_SRCS  += $(call rwildcard, src/, *.cpp)
LOCAL_CPP_SRCS  := $(sort $(LOCAL_CPP_SRCS))
LOCAL_CC_SRCS   += $(call rwildcard, src/, *.cc)
LOCAL_CC_SRCS   := $(sort $(LOCAL_CC_SRCS))
LOCAL_PDE_SRCS  += $(call rwildcard, src/, *.$(SUFFIX))
LOCAL_AS_SRCS   += $(call rwildcard, src/, *.S)
...
ifndef SERIAL_ARGS
SERIAL_ARGS = $(SERIAL_BAUDRATE_FLAG) $(SERIAL_BAUDRATE) $(SERIAL_PORT_FLAG) $(SERIAL_PORT)
endif
